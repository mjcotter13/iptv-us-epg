#!/usr/bin/env python3
"""
Custom IPTV-org US EPG Builder — Version 7.1
...
UA = "Mozilla/5.0 Custom-IPTV-EPG-Builder/7.1"
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    print("Loading V7 additive guides...", flush=True)
    additive_sources = []

    # V7: exact-ID schedules generated by the official IPTV-org grabber.
    targeted_guide = Path("v7_targeted_guide.xml")
    if targeted_guide.exists() and targeted_guide.stat().st_size > 100:
        print("  IPTV-org targeted V7", flush=True)
        try:
            root = ET.parse(targeted_guide).getroot()
            channels = {}
            programmes = defaultdict(list)
            for ch in root.findall("channel"):
                sid = (ch.get("id") or "").strip()
            ...
            additive_sources.append({
                "name": "IPTV-org targeted V7",
                "channels": channels,
                "programmes": programmes,
            })
            print(f"    {len(channels)} targeted channels loaded", flush=True)
        except Exception as exc:
            print(f"  WARNING: skipped targeted V7 guide: {exc}", flush=True)
    ...
    print(f"V7 additive sources added {len(additive_matches)} new matches.", flush=True)
    ...
    tv = ET.Element(
        "tv",
        {
            "generator-info-name": "Custom IPTV-org US EPG Builder v7.1",
    ...
    with open(OUTDIR / "status.txt", "w", encoding="utf-8") as f:
        ...
        f.write(f"targeted_v7_matches={sum(1 for m in additive_matches.values() if m["source"] == "IPTV-org targeted V7")}\n")
